...
export default {
  ...
  mounted () {
    http({
      url: `/gateway?cityId=${this.$store.state.cityId}&pageNum=1&pageSize=10&type=1&k=7093899`,
      headers: {
  ...
  methods: {
    onLoad () {
      ...
      this.curren++
      http({
        url: `/gateway?cityId=${this.$store.state.cityId}&pageNum=${this.curren}&pageSize=10&type=1&k=7093899`,
        headers: {
          'X-Host': 'mall.film-ticket.film.list'
        }
      }).then((res) => {
